...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      ...
      try {
        let res = await this.$store.dispatch("detail/addOrUpdateShopCart", {
          skuId: cart.skuId,
          skuNum: disNum,
        });
        if (res === "ok") {
    ...
    async deleteAllCheckedCart() {
      //删除所有选中的购物
      try {
        await this.$store.dispatch("shopCart/deleteAllChechedCart");
      } catch (error) {
        alert(error.message);
      }
      this.getData();
    ...
    async allChecked(event) {
      try {
        await this.$store.dispatch("shopCart/allChecked",event.target.checked?'1':'0');
        this.getData()
      } catch (error) {
        alert(error.messge);
      }
  ...
  computed: {
    ...
    total() {
      ...
      this.cartInfoList.forEach((element) => {
        if (element.isChecked) sum += element.skuPrice * element.skuNum;
      });
      return sum;
      /*     let res=  this.cartInfoList?.reduce((pre,cur) => {
        return pre.skuPrice * pre.skuNum + cur.skuPrice * cur.skuNum
      })
      return res */
    },
    isAllChecked() {
